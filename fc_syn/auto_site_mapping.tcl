...
while {[gets $fpin line] >= 0} {
    ...
    if {$Site_name != ""} {
        if {!$close_count} {
            if {[regexp {width}  $line]} { set WIDTH  [expr [lindex $line 2]*1] }
            if {[regexp {height} $line]} { set HEIGHT [expr [lindex $line 2]*1] }
        } else {
...
foreach LEF $LEF_DIRS {
    ...
    while {[gets $fpin line] >= 0} {
        ...
        if {$Site_name != ""} {
            ...
            if {[regexp {SIZE} $line]} {
                set WIDTH  [expr [lindex $line 1]*1]
                set HEIGHT [expr [lindex $line 3]*1]
            }
        }
...
if {[info exists Site_map] && $Site_map != ""} {
	...
	puts "Information_ADF: Auto defined DEF site mapping - $Site_map"
    return $Site_map
} else {
